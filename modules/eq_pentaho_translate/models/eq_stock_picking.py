...
class eq_stock_picking(models.Model):
    ...
    def _set_plain_text(self):
        ...
        """
        for rec in self:
            rec.eq_header_text_plaintext = self._remove_wrong_chars(html2text.html2text(rec.eq_header_text))
            rec.eq_footer_plaintext = self._remove_wrong_chars(html2text.html2text(rec.eq_footer_text))